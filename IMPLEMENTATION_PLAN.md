...
- Add a chart alongside the results table for query shapes where one adds clarity beyond raw numbers -- e.g. ARM by region needs a bar chart so the regions can be compared at a glance, not just read off a table.
- Pick the chart type from the actual shape of the returned columns, not something the LLM decides: one categorical dimension + one measure (e.g. ARM by region) -> horizontal bar chart, sorted by value; a time dimension (`metric_month`) + one measure (e.g. churn rate by month) -> line chart; anything else (multi-dimension breakdowns, single-value answers) stays table-only rather than forcing a chart where one doesn't help.
- Decide the rendering approach against the UI's existing no-build-step, no-new-dependency constraint: a CDN-loaded charting library vs. hand-rolled Canvas/SVG bars. Went with a CDN-loaded Chart.js.
- Reuse the existing CSS tokens (accent, surface, text-dim) so the chart matches the rest of the page rather than introducing its own visual language.
- Test the chart-type heuristic against representative query shapes -- including ones that should stay table-only -- before trusting it end-to-end.
- Add a pie chart for percentage/proportion-of-whole results (e.g. "percentage of subscribers by region"), detected the same data-driven way -- a numeric column whose values sum to ~100 (or ~1), or whose name matches a percent/share/proportion pattern. Live-tested first and caught a real gap: the model consistently returns *both* the raw aggregate and the percentage as separate columns (e.g. `region_id, active_subscribers, pct_of_active_subscribers`), not just dimension + percentage -- so the two-column-only heuristic from the bar/line case would have silently left every real percentage question table-only. Extended detection to tolerate one extra numeric column and pick the percentage-shaped one as the chart measure.
